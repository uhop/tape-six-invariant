...
- **`check(cond, message)`** reads the live slot per call. With a host: it reports `{ok, message, marker: new Error()}` so the assertion lands on the current test with source location at the call site. Without a host: on failure it runs the configured **absent behavior** (default `inert`).
...
- **Absent behavior** is a single generic setter (`setAbsentBehavior(fn)`), not an env-flag menu. Canned behaviors ship as exported functions: `inert` (default), `throwOnFail`, `warnOnFail`. There is deliberately no canned `node:assert` behavior — a synchronous `check` should not dynamically import, so deferring to `node:assert` is a bring-your-own-import one-liner (`setAbsentBehavior((ok, message) => assert.ok(ok, message))`), keeping the package zero-dependency.
